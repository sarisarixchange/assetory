<template>
    ...
    <svg class="svgDefaultButton" viewBox="0 0 379 32" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg"
        @mouseenter="isHovered = true" @mouseleave="isHovered = false">

        <!-- 2. Añadimos vector-effect="non-scaling-stroke" para que el borde no se engrose al estirarse -->
        <path class="oddShapeButton" :fill="(isHovered || isFocused) ? hoverColor : fillColor"
            :stroke="isHovered || isFocused ? hoverStrokeColor : strokeColor" :stroke-width="isFocused ? 3 : 1"
        ...
        <foreignObject x="0" y="0" width="100%" height="100%">
            <!-- Render as router-link if 'to' exists -->
            <router-link v-if="to" :to="to" class="button-text" @focus="isFocused = true" @blur="isFocused = false"
            ...
            <!-- Fallback to plain button/div for click events -->
            <button v-else type="button" class="button-text" @focus="isFocused = true" @blur="isFocused = false"
                :aria-label="label + ' ' + (iconAlt || '')"
                :style="{ color: (isHovered || isFocused) ? hoverTextColor : textColor }">
                <span class="label">{{ label }}</span>
                <img v-if="iconSrc" class="icon-img" :src="isHovered ? hoverIconSrc : iconSrc" :alt="iconAlt" />
...
const props = defineProps({
    ...
    hoverStrokeColor: { type: String, default: 'var(--homepage-left-card-button-border-color)' },
